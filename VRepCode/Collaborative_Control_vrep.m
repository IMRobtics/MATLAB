...
[~,pos]=vrep.simxGetObjectPosition(clientID, r1_handle, -1, vrep.simx_opmode_blocking);
robot{1}=[pos(1)*mult,pos(2)*mult];

[~,r2_handle]=vrep.simxGetObjectHandle(clientID,'K3_robot#0', vrep.simx_opmode_blocking);
[~,pos]=vrep.simxGetObjectPosition(clientID, r2_handle, -1, vrep.simx_opmode_blocking);
robot{2}=[pos(1)*mult,pos(2)*mult];

[~,r3_handle]=vrep.simxGetObjectHandle(clientID,'K3_robot#1', vrep.simx_opmode_blocking);
[~,pos]=vrep.simxGetObjectPosition(clientID, r3_handle, -1, vrep.simx_opmode_blocking);
robot{3}=[pos(1)*mult,pos(2)*mult];

[~,r4_handle]=vrep.simxGetObjectHandle(clientID,'K3_robot#2', vrep.simx_opmode_blocking);
[~,pos]=vrep.simxGetObjectPosition(clientID, r4_handle, -1, vrep.simx_opmode_blocking);
robot{4}=[pos(1)*mult,pos(2)*mult];
...
for t=1:25  %total simulation time    
...
%     Electrostatic_Forces
    ...
    for i=1:number_of_robots
        ...
        elseif (abs(distance_from_hyp_robot-alpha)<=5e-1)
            ...
            % need to CONVERT this to VLEFT and VRIGHT of robots
            % Velocity MAX = 2*pi
